...
class ModelProvider:
    def __init__(
        ...
        dummy_index = pd.date_range(
            start=pd.Timestamp(2000, 1, 1), freq="1h", periods=8 * 24
        )
        dummy_data = pd.DataFrame(
            index=dummy_index, columns=["power"], data=np.zeros(shape=(8 * 24))
        )
        dummy_series = TimeSeries.from_dataframe(dummy_data)
        dummy_covar = TimeSeries.from_dataframe(
            encode_calendar(index=dummy_index, future_time_steps=25)
        )

        self.model_path = model_path
        self.model = self.recreate_model(
            target_series=dummy_series, covariates=dummy_covar
        )
        self.n_samples = n_samples

    def fit(
        self, training_data_path: Union[Path, str], fitted_model_path: Union[Path, str]
    ):
        ...
        self.model.fit(
            series=subseries,
            future_covariates=subseries_covar,
            epochs=1,
...
def _encode_data(
    data: pd.DataFrame, future_time_steps=25, lower_bound_zero=True
) -> Tuple[Scaler, TimeSeries, TimeSeries]:
    ...
    future_covar = TimeSeries.from_dataframe(
        encode_calendar(index=ts.time_index, future_time_steps=future_time_steps)
    )
    return scaler, scaled_ts, future_covar


if __name__ == "__main__":
    data_path = project_root.joinpath(
        "sample_data/ccef3bd47b7d8a93578791579362d7f61e6daed3dd4acbab47997bc8927218f.csv"
    )
